...
import { Badge, SegmentedControl } from '@sagtech-infra/ui';
...
// Scoped light/dark preview. The `data-theme` attribute on the frame flips the
// semantic tokens (theme.css targets `[data-theme="light"]`, inheriting down),
// so the same live template renders in either theme without touching the page.
const THEME_OPTIONS = [
...
/**
 * Homepage hero of the templates feature: pick a full-page template on the left,
 * see a live, scaled preview on the right — and flip the preview between the
 * dark and the warm light theme right on the page. Adapted from the /templates
 * TemplatesShowcase, with an added scoped theme toggle.
 */
export function HomeTemplateShowcase() {
  ...
  return (
    <motion.div
      ...
      className="grid gap-24px lg:grid-cols-[300px_1fr]"
    >
      {/* ───────── list ───────── */}
      <div className="flex flex-col gap-20px">
                    ...
                    <span
                      ...
                      className={`text-14 text-pr_purple transition-all duration-200 ${
                        isActive
                          ...
                          : '-translate-x-[6px] opacity-0 group-hover:opacity-50'
                      }`}
                    >
                      →
                    </span>
        ...
        <p className="px-4px text-12 text-grey_3">
          {templates.length} full-page templates · hover to preview, flip the theme, open for the code
        </p>
      </div>

      {/* ───────── preview ───────── */}
      <div className="flex flex-col gap-16px">
        {/* `data-theme` scopes the whole frame (chrome + preview) to the chosen
            theme — the dual-theme demo. The page itself stays dark. */}
        <div
          data-theme={mode}
          className="relative overflow-hidden rounded-16px border border-border-default bg-bg-secondary shadow-3xl"
        >
          ...
          <div className="relative z-10 flex items-center gap-12px border-b border-border-default bg-bg-primary px-16px py-10px">
            ...
            <div className="hidden min-w-0 flex-1 truncate rounded-8px bg-bg-tertiary px-12px py-4px text-center font-mono text-12 text-grey_3 sm:block">
          ...
          {/* scaled live preview — the click target is an overlay link (below),
              kept OUT of the anchor so the template's own <a>s don't nest inside
              another <a> (invalid HTML → hydration error). */}
          <div
            ref={frameRef}
            className="relative z-10 h-[300px] overflow-hidden bg-bg-primary sm:h-[420px] lg:h-[480px]"
          >
              ...
              <NextLink
                ...
                className="absolute inset-0 z-20 transition-colors hover:bg-white/[0.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pr_purple"
              />
            </div>
        </div>
